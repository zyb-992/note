...
1. 散列函数以用户数据m作为输入，并计算得到一个称为散列的固定长度的字符串H(m)
2. 不可能找得到两个不同的报文x和y，使得H(x)=H(y)
...
3. 主要是通过私钥对报文进行加密，对方再使用公钥对加密了的报文进行解密，因为私钥是唯一的，加密后的报文若被篡改，则不能通过公钥进行解密，所以保证了报文的唯一性和完整性
